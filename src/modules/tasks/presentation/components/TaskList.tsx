...
import { getTaskSlaStatus, type TaskPage, type WorkTask } from "@/modules/tasks/domain/task";

const emptyPage: TaskPage = { tasks: [], page: 1, pageSize: 10, total: 0, totalPages: 0 };
const slaLabel = (task: WorkTask) => ({ "without-deadline": null, "on-time": "No prazo", "due-soon": "Vence em até 24h", overdue: "Atrasada", completed: "Concluída" }[getTaskSlaStatus(task)]);
...
export function TaskList() {
  ...
  return <div className="space-y-5">
    ...
    {!loading && !error && result.tasks.length > 0 ? <><p className="text-sm text-slate-500">{result.total} {result.total === 1 ? "tarefa encontrada" : "tarefas encontradas"}</p><ul className="grid gap-4">{result.tasks.map((task) => { const sla = slaLabel(task); return <li key={task.id}><Link className="block rounded-lg border border-slate-200 bg-white p-5 hover:border-brand-400" href={`/tasks/${task.id}`}><div className="flex justify-between gap-4"><strong>{task.stepName}</strong><div className="flex flex-wrap gap-2">{sla ? <span className={`rounded-full px-2 py-1 text-xs font-semibold ${sla === "Atrasada" ? "bg-rose-100 text-rose-800" : sla === "Vence em até 24h" ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{sla}</span> : null}<span className={`rounded-full px-2 py-1 text-xs font-semibold ${task.status === "running" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-700"}`}>{task.status === "running" ? "Em andamento" : "Pendente"}</span></div></div><p className="mt-2 text-sm text-slate-600">{task.workflowName} · {task.organizationName}</p><p className="mt-1 text-xs text-slate-500">Responsável: {task.assigneeName} · Criada em {new Date(task.createdAt).toLocaleString("pt-BR")}{task.dueAt ? ` · Vence em ${new Date(task.dueAt).toLocaleString("pt-BR")}` : ""}</p></Link></li>; })}</ul><nav aria-label="Paginação da fila" className="flex items-center justify-between"><button className="rounded-md border px-3 py-2 text-sm disabled:opacity-40" disabled={result.page <= 1} onClick={() => setPage((current) => current - 1)} type="button">Anterior</button><span className="text-sm text-slate-600">Página {result.page} de {Math.max(1, result.totalPages)}</span><button className="rounded-md border px-3 py-2 text-sm disabled:opacity-40" disabled={result.page >= result.totalPages} onClick={() => setPage((current) => current + 1)} type="button">Próxima</button></nav></> : null}
  </div>;
}
