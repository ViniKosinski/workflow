...
export function WorkflowForm() {
  ...
  return <form className="space-y-5" onSubmit={handleSubmit}>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-4"><div><h2 className="font-bold text-slate-950">Mapa do processo</h2><p className="mt-1 text-xs text-slate-500">Arraste a área vazia com a mão para navegar. Selecione uma atividade para configurá-la.</p></div><div className="flex gap-2 text-xs"><span className="rounded bg-emerald-50 px-2 py-1 text-emerald-700">Início</span><span className="rounded bg-amber-50 px-2 py-1 text-amber-700">Atividade</span><span className="rounded bg-violet-50 px-2 py-1 text-violet-700">Resultados</span></div></header>
        <div className="min-h-[470px] cursor-grab touch-none select-none overflow-auto p-6 active:cursor-grabbing sm:p-10" onPointerCancel={panEnd} onPointerDown={panStart} onPointerMove={panMove} onPointerUp={panEnd} ref={canvasRef} style={{ backgroundColor: "#f8fafc", backgroundImage: "linear-gradient(#e2e8f0 1px, transparent 1px), linear-gradient(90deg, #e2e8f0 1px, transparent 1px)", backgroundSize: "24px 24px" }}><div className="mx-auto flex min-w-0 flex-col items-center justify-center md:min-h-[380px] md:w-max md:min-w-full md:flex-row">
