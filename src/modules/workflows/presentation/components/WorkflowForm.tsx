...
const newOutcome = (): WorkflowFormOutcome => ({ id: crypto.randomUUID(), name: "Concluir", targetStepId: null, targetEndId: "end-1" });
...
type ConnectionPath = { id: string; from: Point; to: Point; kind: "step" | "end" };
...
export function WorkflowForm() {
  ...
  const [endNodes, setEndNodes] = useState([{ id: "end-1", name: "Fim 1" }]);
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const point = (element: Element, side: "left" | "right" | "top"): Point => { const rect = element.getBoundingClientRect(); return side === "top" ? { x: rect.left + rect.width / 2 - canvasRect.left + canvas.scrollLeft, y: rect.top - canvasRect.top + canvas.scrollTop } : { x: rect[side] - canvasRect.left + canvas.scrollLeft, y: rect.top + rect.height / 2 - canvasRect.top + canvas.scrollTop }; };
      setConnectionPaths(steps.flatMap((step) => step.outcomes.flatMap((outcome) => {
        ...
        const target = outcome.targetStepId ? canvas.querySelector(`[data-step-id="${outcome.targetStepId}"]`) : !outcome.externalWorkflowId ? canvas.querySelector(`[data-end-id="${outcome.targetEndId ?? endNodes[0]?.id}"]`) : null;
        if (!target) return [];
        return [{ id: outcome.id, from: point(source, "right"), to: point(target, outcome.targetStepId ? "left" : "top"), kind: outcome.targetStepId ? "step" as const : "end" as const }];
      })));
    };
  ...
  }, [steps, availableWorkflows, zoom, endNodes]);
  ...
  useEffect(() => {
    ...
    const finish = (event: globalThis.PointerEvent) => {
      ...
      const endId = target?.closest<HTMLElement>("[data-end-id]")?.dataset.endId;
      if (targetStep && targetStep !== linkDrag.stepId) updateOutcomeForStep(linkDrag.stepId, linkDrag.outcomeId, { targetStepId: targetStep, targetEndId: undefined, externalWorkflowId: undefined, externalStepId: undefined });
      else if (endId) updateOutcomeForStep(linkDrag.stepId, linkDrag.outcomeId, { targetStepId: null, targetEndId: endId, externalWorkflowId: undefined, externalStepId: undefined });
      setLinkDrag(null);
    };
  ...
  function destinationValue(outcome: WorkflowFormOutcome) { return outcome.externalWorkflowId && outcome.externalStepId ? `external:${outcome.externalWorkflowId}:${outcome.externalStepId}` : outcome.targetStepId ? `local:${outcome.targetStepId}` : `end:${outcome.targetEndId ?? endNodes[0]?.id}`; }
  function changeDestination(outcomeId: string, value: string) {
    if (value.startsWith("local:")) updateOutcome(outcomeId, { targetStepId: value.slice(6), targetEndId: undefined, externalWorkflowId: undefined, externalStepId: undefined });
    else if (value.startsWith("external:")) { const [, workflowId, stepId] = value.split(":"); updateOutcome(outcomeId, { targetStepId: null, targetEndId: undefined, externalWorkflowId: workflowId, externalStepId: stepId }); }
    else if (value.startsWith("end:")) updateOutcome(outcomeId, { targetStepId: null, targetEndId: value.slice(4), externalWorkflowId: undefined, externalStepId: undefined });
  }
  function destinationLabel(outcome: WorkflowFormOutcome) { if (outcome.externalWorkflowId) return availableWorkflows.find((flow) => flow.id === outcome.externalWorkflowId)?.name ?? "Outro fluxo"; return outcome.targetStepId ? steps.find((step) => step.id === outcome.targetStepId)?.name : endNodes.find((end) => end.id === (outcome.targetEndId ?? endNodes[0]?.id))?.name ?? "Fim"; }
  function addEndNode() { const id = crypto.randomUUID(); setEndNodes((current) => [...current, { id, name: `Fim ${current.length + 1}` }]); }
