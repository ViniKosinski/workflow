...
export function WorkflowForm() {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      setConnectionPaths(steps.flatMap((step) => step.outcomes.flatMap((outcome) => {
        ...
        const target = outcome.targetStepId ? canvas.querySelector(`[data-step-id="${outcome.targetStepId}"]`) : !outcome.externalWorkflowId ? canvas.querySelector(`[data-end-outcome-id="${outcome.id}"]`) : null;
        if (!target) return [];
  ...
  useEffect(() => {
    ...
    const finish = (event: globalThis.PointerEvent) => {
      ...
      const ends = Boolean(target?.closest("[data-end-node], [data-end-outcome-id]"));
